...
public class FloridaPage {

    String state = HomesSubMenus.FLORIDA.getState();
    String stateCode = HomesSubMenus.FLORIDA.getStateCode();

    @FindBy(css = "h1.prohibition")
    private SelenideElement heading;

    public FloridaPage verifyTitle(){
        assertTrue(title().contains(state), "Title does not contains " + state);
        return this;
    }

    public FloridaPage verifyHeading(){
        heading.shouldHave(Condition.text(state));
        return this;
    }

    public FloridaPage verifyURL(){
